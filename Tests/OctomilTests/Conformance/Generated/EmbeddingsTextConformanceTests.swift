...
//   - LLAMA_CPP_OFF: runtime built with -DOCT_ENABLE_ENGINE_LLAMA_CPP=OFF
//   - MODEL_ARTIFACT_UNSET: OCTOMIL_LLAMA_CPP_MODEL env var unset
//
...
final class EmbeddingsTextConformanceTests: XCTestCase {

    // MARK: - Capability Name

    func testEmbeddingsTextCapabilityNameConstant() {
        // Byte-for-byte match against contract YAML `capability:` field.
        XCTAssertEqual("embeddings.text", "embeddings.text",
            "capability name literal must match contract byte-for-byte")
    }

    // MARK: - Bounded Error Codes

    func testEmbeddingsTextBoundedErrorCodes() {
        // bounded_error_codes from conformance/embeddings.text.yaml
        XCTAssertNotNil(ErrorCode(rawValue: "invalid_input"), "bounded error code invalid_input missing from ErrorCode enum")
        XCTAssertNotNil(ErrorCode(rawValue: "context_too_large"), "bounded error code context_too_large missing from ErrorCode enum")
    ...
    func testEmbeddingsTextAllowedEventSetWellFormed() {
        // Contract: emitted event types ⊆ expected_event_sequence ∪ runtime_scope_events
        // Source: conformance/embeddings.text.yaml expected_event_sequence
        ...
        XCTAssertFalse(allowedEvents.isEmpty,
            "embeddings.text: allowed event set must not be empty")
    ...
    func testEmbeddingsTextLifecycleSteps() {
        // Source: conformance/embeddings.text.yaml lifecycle field
        ...
        XCTAssertEqual(steps.first, "runtime_open",
            "embeddings.text: lifecycle must begin with runtime_open")
        XCTAssertEqual(steps.last, "runtime_close",
            "embeddings.text: lifecycle must end with runtime_close")
        if steps.contains("model_open") {
            ...
            XCTAssertLessThan(modelOpenIdx, sessionOpenIdx,
                "embeddings.text: model_open must precede session_open")
        }
    }

    // MARK: - Privacy Constraints

    func testEmbeddingsTextDenyFieldSubstrings() {
        // deny_field_substrings from conformance/embeddings.text.yaml
    ...
    func testEmbeddingsTextLifecycle() throws {
        throw XCTSkip(
            "SKIP_WITH_EXPLICIT_REASON: native lifecycle for \"embeddings.text\" requires " +
        ...
        // 2. oct_runtime_capabilities → assert "embeddings.text" advertised
        guard ProcessInfo.processInfo.environment["OCTOMIL_LLAMA_CPP_MODEL"] != nil else {
            throw XCTSkip("SKIP: required env var OCTOMIL_LLAMA_CPP_MODEL is unset — capability not advertised by runtime")
        }
        ...
        // 5. Assert terminal_statuses ∈ ['OCT_STATUS_OK', 'OCT_STATUS_INVALID_INPUT', 'OCT_STATUS_CANCELLED']
    }

    func testEmbeddingsTextNoSilentCloudFallback() throws {
        throw XCTSkip(
            "SKIP_WITH_EXPLICIT_REASON: native path for \"embeddings.text\" not exercisable " +
