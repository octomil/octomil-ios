...
final class AudioTtsBatchConformanceTests: XCTestCase {

    // MARK: - Capability Name

    func testAudioTtsBatchCapabilityNameConstant() {
        // Byte-for-byte match against contract YAML `capability:` field.
        XCTAssertEqual("audio.tts.batch", "audio.tts.batch",
    ...
    func testAudioTtsBatchBoundedErrorCodes() {
        // bounded_error_codes from conformance/audio.tts.batch.yaml
    ...
    func testAudioTtsBatchAllowedEventSetWellFormed() {
        // Contract: emitted event types ⊆ expected_event_sequence ∪ runtime_scope_events
        // Source: conformance/audio.tts.batch.yaml expected_event_sequence
        ...
        XCTAssertFalse(allowedEvents.isEmpty,
            "audio.tts.batch: allowed event set must not be empty")
    ...
    func testAudioTtsBatchLifecycleSteps() {
        // Source: conformance/audio.tts.batch.yaml lifecycle field
        ...
        XCTAssertEqual(steps.first, "runtime_open",
            "audio.tts.batch: lifecycle must begin with runtime_open")
        XCTAssertEqual(steps.last, "runtime_close",
            "audio.tts.batch: lifecycle must end with runtime_close")
        if steps.contains("model_open") {
            ...
            XCTAssertLessThan(modelOpenIdx, sessionOpenIdx,
                "audio.tts.batch: model_open must precede session_open")
        }
    }

    // MARK: - Privacy Constraints

    func testAudioTtsBatchDenyFieldSubstrings() {
        // deny_field_substrings from conformance/audio.tts.batch.yaml
    ...
    func testAudioTtsBatchLifecycle() throws {
        throw XCTSkip(
            "SKIP_WITH_EXPLICIT_REASON: native lifecycle for \"audio.tts.batch\" requires " +
        ...
        // 2. oct_runtime_capabilities → assert "audio.tts.batch" advertised
    ...
    func testAudioTtsBatchNoSilentCloudFallback() throws {
        throw XCTSkip(
            "SKIP_WITH_EXPLICIT_REASON: native path for \"audio.tts.batch\" not exercisable " +
