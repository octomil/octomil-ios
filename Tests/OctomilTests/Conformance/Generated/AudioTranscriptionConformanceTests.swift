...
//   - WHISPER_CPP_OFF: runtime built with -DOCT_ENABLE_ENGINE_WHISPER_CPP=OFF
//   - WHISPER_BIN_UNSET: OCTOMIL_WHISPER_BIN env var unset (capability not advertised)
//   - WHISPER_BIN_BAD_DIGEST: OCTOMIL_WHISPER_BIN points at file with non-canonical SHA-256
//
...
final class AudioTranscriptionConformanceTests: XCTestCase {

    // MARK: - Capability Name

    func testAudioTranscriptionCapabilityNameConstant() {
        // Byte-for-byte match against contract YAML `capability:` field.
        XCTAssertEqual("audio.transcription", "audio.transcription",
    ...
    func testAudioTranscriptionBoundedErrorCodes() {
        // bounded_error_codes from conformance/audio.transcription.yaml
    ...
    func testAudioTranscriptionAllowedEventSetWellFormed() {
        // Contract: emitted event types ⊆ expected_event_sequence ∪ runtime_scope_events
        // Source: conformance/audio.transcription.yaml expected_event_sequence
        ...
        XCTAssertFalse(allowedEvents.isEmpty,
            "audio.transcription: allowed event set must not be empty")
    ...
    func testAudioTranscriptionLifecycleSteps() {
        // Source: conformance/audio.transcription.yaml lifecycle field
        ...
        XCTAssertEqual(steps.first, "runtime_open",
            "audio.transcription: lifecycle must begin with runtime_open")
        XCTAssertEqual(steps.last, "runtime_close",
            "audio.transcription: lifecycle must end with runtime_close")
        if steps.contains("model_open") {
            ...
            XCTAssertLessThan(modelOpenIdx, sessionOpenIdx,
                "audio.transcription: model_open must precede session_open")
        }
    }

    // MARK: - Privacy Constraints

    func testAudioTranscriptionDenyFieldSubstrings() {
        // deny_field_substrings from conformance/audio.transcription.yaml
    ...
    func testAudioTranscriptionLifecycle() throws {
        throw XCTSkip(
            "SKIP_WITH_EXPLICIT_REASON: native lifecycle for \"audio.transcription\" requires " +
        ...
        // 2. oct_runtime_capabilities → assert "audio.transcription" advertised
        guard ProcessInfo.processInfo.environment["OCTOMIL_WHISPER_BIN"] != nil else {
            throw XCTSkip("SKIP: required env var OCTOMIL_WHISPER_BIN is unset — capability not advertised by runtime")
        }
    ...
    func testAudioTranscriptionNoSilentCloudFallback() throws {
        throw XCTSkip(
            "SKIP_WITH_EXPLICIT_REASON: native path for \"audio.transcription\" not exercisable " +
