...
final class ChatCompletionConformanceTests: XCTestCase {

    // MARK: - Capability Name

    func testChatCompletionCapabilityNameConstant() {
        // Byte-for-byte match against contract YAML `capability:` field.
        XCTAssertEqual("chat.completion", "chat.completion",
    ...
    func testChatCompletionBoundedErrorCodes() {
        // bounded_error_codes from conformance/chat.completion.yaml
    ...
    func testChatCompletionAllowedEventSetWellFormed() {
        // Contract: emitted event types ⊆ expected_event_sequence ∪ runtime_scope_events
        // Source: conformance/chat.completion.yaml expected_event_sequence
    ...
    func testChatCompletionLifecycleSteps() {
        // Source: conformance/chat.completion.yaml lifecycle field
        ...
        XCTAssertEqual(steps.first, "runtime_open",
            "chat.completion: lifecycle must begin with runtime_open")
        XCTAssertEqual(steps.last, "runtime_close",
            "chat.completion: lifecycle must end with runtime_close")
        if steps.contains("model_open") {
            ...
            XCTAssertLessThan(modelOpenIdx, sessionOpenIdx,
                "chat.completion: model_open must precede session_open")
        }
    }

    // MARK: - Privacy Constraints

    func testChatCompletionDenyFieldSubstrings() {
        // deny_field_substrings from conformance/chat.completion.yaml
    ...
    func testChatCompletionLifecycle() throws {
        throw XCTSkip(
            "SKIP_WITH_EXPLICIT_REASON: native lifecycle for \"chat.completion\" requires " +
    ...
    func testChatCompletionNoSilentCloudFallback() throws {
        throw XCTSkip(
            "SKIP_WITH_EXPLICIT_REASON: native path for \"chat.completion\" not exercisable " +
